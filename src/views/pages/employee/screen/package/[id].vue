...
// Fetch initial package details
const fetchPackageDetail = async () => {
  loading.value = true;
  try {
    const response = await packageStore.show(packageId);
    packageDetail.value = response.data || response;
  } catch (error) {
    console.error('Error fetching package:', error);
    ElMessage.error('Failed to load package details');
  } finally {
    loading.value = false;
  }
};
